...
function readPreciosConDesgloses() {
  try { return JSON.parse(fs.readFileSync(path.join(ROOT, 'precios-con-desgloses.json'), 'utf8')); }
  catch { return { items: [], recursos: { mano_obra: [], materiales: [], maquinaria: [] } }; }
}

// Mapeo de capítulos por oficios a categorías BCEXTREM
const CAPITULOS_MAP = {
  '01': ['DEMOLICION', 'R0'],
  '02': ['CARGA'],
  '03': ['A0'],
  '04': ['N0'],
  '05': ['AISLAMIENTO'],
  '06': ['E1', 'U0'],  // E1 y U0 contienen instalaciones (electricidad, fontanería, climatización)
  '06.1': [],
  '06.2': [],
  '06.3': [],
  '07': [],
...
function getCategoriasFromCapitulo(capitulo) {
  return CAPITULOS_MAP[capitulo] || [];
}
...
<body>
  ...
  if (req.method === 'GET' && url.pathname === '/api/precios') {
    ...
    let items = preciosData.items || [];

    if (q) {
      const query = q.toLowerCase();
      items = items.filter(item =>
        item.cod?.toLowerCase().includes(query) ||
        item.res?.toLowerCase().includes(query) ||
  ...
  // API: búsqueda avanzada con método QUERY (RFC 9535)
  if (req.method === 'QUERY' && url.pathname === '/api/precios/search') {
    try {
      const body = await readBody(req);
      const filters = body.filters || {};
      const sort = body.sort || { field: 'cod', order: 'asc' };
      const pagination = body.pagination || { page: 1, limit: 50 };
      ...
      // Filtro por capítulo (convierte capítulo a categorías)
      if (filters.capitulo) {
        const capitulos = Array.isArray(filters.capitulo) ? filters.capitulo : [filters.capitulo];
        const categoriasDelCapitulo = capitulos.flatMap(cap => getCategoriasFromCapitulo(cap));
        if (categoriasDelCapitulo.length > 0) {
          items = items.filter(item => categoriasDelCapitulo.includes(item.categoria));
        }
      }

      // Filtro por categoría directa
      if (filters.categoria) {
        const categorias = Array.isArray(filters.categoria) ? filters.categoria : [filters.categoria];
        items = items.filter(item => categorias.includes(item.categoria));
      }

      if (filters.precio) {
        if (filters.precio.min !== undefined) {
          items = items.filter(item => item.precio >= filters.precio.min);
        }
        if (filters.precio.max !== undefined) {
          items = items.filter(item => item.precio <= filters.precio.max);
        }
      }

      if (filters.unidades) {
        const unidades = Array.isArray(filters.unidades) ? filters.unidades : [filters.unidades];
        items = items.filter(item => unidades.includes(item.uni));
      }

      // Ordenamiento
      const sortField = sort.field || 'cod';
      ...
      res.end(JSON.stringify({ ok: false, error: 'Petición QUERY inválida: ' + error.message }));
      return;
    }
  }

  // API: obtener desglose de una partida
  if (req.method === 'GET' && url.pathname.startsWith('/api/precios/') && url.pathname.endsWith('/desglose')) {
    const codigo = url.pathname.split('/')[3]; // /api/precios/:codigo/desglose
    const data = readPreciosConDesgloses();

    // Buscar la partida
    const partida = data.items.find(item => item.cod === codigo);

    if (!partida) {
      res.writeHead(404, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ ok: false, error: 'Partida no encontrada' }));
      return;
    }

    if (!partida.descomposicion || partida.descomposicion.length === 0) {
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({
        ok: true,
        partida: {
        ...
        desglose: null,
        message: 'Esta partida no tiene desglose disponible'
      }));
      return;
    }

    // Calcular desglose detallado
    let totalMO = 0, totalMAT = 0, totalMAQ = 0;
    const componentes = [];

    partida.descomposicion.forEach(comp => {
      const mo = data.recursos.mano_obra.find(r => r.cod === comp.cod);
      const mat = data.recursos.materiales.find(r => r.cod === comp.cod);
      const maq = data.recursos.maquinaria.find(r => r.cod === comp.cod);
      const recurso = mo || mat || maq;

      if (recurso) {
        const subtotal = comp.cantidad * recurso.precio;
        const tipo = mo ? 'MO' : mat ? 'MAT' : 'MAQ';

        componentes.push({
          cod: recurso.cod,
          descripcion: recurso.res,
          tipo: tipo,
        ...
        if (mo) totalMO += subtotal;
        if (mat) totalMAT += subtotal;
        if (maq) totalMAQ += subtotal;
      }
    });

    const totalDirecto = totalMO + totalMAT + totalMAQ;
    ...
    res.end(JSON.stringify({
      ...
      desglose: {
        ...
        resumen: {
          mano_obra: parseFloat(totalMO.toFixed(2)),
          materiales: parseFloat(totalMAT.toFixed(2)),
          maquinaria: parseFloat(totalMAQ.toFixed(2)),
          total_directo: parseFloat(totalDirecto.toFixed(2)),
          precio_bd: partida.precio,
          indirectos: parseFloat((partida.precio - totalDirecto).toFixed(2)),
          porcentaje_mo: parseFloat((totalMO / totalDirecto * 100).toFixed(1)),
          porcentaje_mat: parseFloat((totalMAT / totalDirecto * 100).toFixed(1)),
          porcentaje_maq: parseFloat((totalMAQ / totalDirecto * 100).toFixed(1))
        }
      },
      source: 'BCEXTREM 2026 (BC3 con desgloses)'
    }));
    return;
  }
